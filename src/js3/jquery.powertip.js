...
(function(root, factory) {
	...
	function PlacementCalculator() {
		...
		function computePlacementCoords(element, placement, tipWidth, tipHeight, offset) {
			...
			switch (placement) {
				...
				case 'e':
				case 'w':
					if(placement === 'e') coords.set('left', position.left + offset);
					else coords.set('right', session.windowWidth - position.left + offset);
					
					var wintop = $(window).scrollTop();
					if((session.windowHeight - tipHeight) > 10)  {
						//clamp tooltip position window's top-bottom 
						coords.set('top', Math.min(Math.max(position.top - (tipHeight / 2), wintop + 10), wintop + session.windowHeight - tipHeight - 20));
					}  else  coords.set('top', wintop + 10);
					break;
				case 's':
					coords.set('left', position.left - (tipWidth / 2));
	...
	function TooltipController(options) {
		...
		if(options.css) {
			//tipElement.css('max-width', options.maxWidth);
			tipElement.css(options.css);
		}
		...
		function closeDesyncedTip() {
			...
			if (session.isTipOpen && !session.isClosing && !session.delayInProgress && ($.inArray('mouseleave', options.closeEvents) > -1 || $.inArray('mouseout', options.closeEvents) > -1 || $.inArray('blur', options.closeEvents) > -1 || $.inArray('focusout', options.closeEvents) > -1)) {
				...
				//if (session.activeHover && (session.activeHover.data(DATA_HASACTIVEHOVER) === false || session.activeHover.is(':disabled'))) {
				if (session.activeHover)  {
