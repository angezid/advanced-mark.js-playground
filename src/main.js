	...
	loadJson : function() {
		...
		if(str) {
			...
			if(type && types[type]) {
				...
				if(types[type].testEditorState === 'text') {
					runCode();
				}
...
function switchLibrary(elem) {
	...
	if($(elem).prop('checked')) {
		oldLibrary = false;
		jqueryMark = info.jquery === 'new' || info.javascript === 'old';
		
	} else {
		oldLibrary = true;
		jqueryMark = info.jquery === 'old' || info.javascript === 'new';
	}
...
const highlighter = {
	...
	highlightRawHtml : function(selector) {
		const elem = markElement,
			// matches the mark elements without or with one nested element
