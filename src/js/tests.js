...
const test = {
	jsons : {
		string_ : [
			`{"section":{"type":"string_","element":"span","className":"test","exclude":"['h1','.ignore']","accuracy":"{value:'exactly',limiters:[',','.']}","diacritics":false,"synonyms":"{'one':'1'}","iframes":true,"iframesTimeout":4999,"acrossElements":true,"caseSensitive":true,"ignoreJoiners":true,"ignorePunctuation":"[',','.']","wildcards":"enabled","blockElementsBoundary":"{tagNames:['div','p'],char:'.'}","combinePatterns":101,"cacheTextNodes":true,"wrapAllRanges":true,"shadowDOM":"{style:'mark[data-markjs]{color:red;}'}","debug":true,"queryString":"query test","selectors":"p,li","selectorAll":true,"testString":{"mode":"html","content":"<p>Test string.</p>"}}}`,
			
			`{"section":{"type":"string_","cacheTextNodes":true,"queryString":"query test","testString":{"mode":"html","content":"<p>Test string.</p>"}}}`,
			
			`{"section":{"type":"string_","combinePatterns":1,"shadowDOM":true,"queryString":"query test","testString":{"mode":"html","content":"<p>Test string.</p>"}}}`,
			
			`{"section":{"type":"string_","acrossElements":true,"blockElementsBoundary":true,"queryString":"query test","testString":{"mode":"html","content":"<p>Test string.</p>"}}}`,
		],
		array : [
			`{"section":{"type":"array","element":"span","className":"test","exclude":"['h1','.ignore']","accuracy":"{value:'exactly',limiters:[',','.']}","diacritics":false,"synonyms":"{'one':'1'}","iframes":true,"iframesTimeout":4999,"acrossElements":true,"caseSensitive":true,"ignoreJoiners":true,"ignorePunctuation":"[',','.']","wildcards":"enabled","blockElementsBoundary":"{tagNames:['div','p'],char:'.'}","combinePatterns":101,"cacheTextNodes":true,"wrapAllRanges":true,"shadowDOM":"{style:'mark[data-markjs]{color:red;}'}","debug":true,"queryArray":"['query','test']","selectors":"p,li","selectorAll":true,"testString":{"mode":"html","content":"<p>Test string.</p>"}}}`,
			`{"section":{"type":"array","acrossElements":true,"blockElementsBoundary":true,"combinePatterns":10,"cacheTextNodes":true,"wrapAllRanges":true,"shadowDOM":"{style:'mark[data-markjs]{color:red;}'}","debug":true,"queryArray":"['query','test']","selectors":"p,li","selectorAll":true,"testString":{"mode":"html","content":"<p>Test string.</p>"}}}`,
		],
		regexp : [
			`{"section":{"type":"regexp","element":"span","className":"test","exclude":"['h1','.ignore']","iframes":true,"iframesTimeout":4999,"acrossElements":true,"separateGroups":true,"blockElementsBoundary":"{tagNames:['div','p'],char:'.'}","wrapAllRanges":true,"shadowDOM":"{style:'mark[data-markjs]{color:red;}'}","debug":true,"queryRegExp":"/\\\\b(?:query|test)\\\\b/gi","selectors":"p,li","selectorAll":true,"testString":{"mode":"html","content":"<p>Test string.</p>"}}}`,
			`{"section":{"type":"regexp","element":"span","className":"test","exclude":"['h1','.ignore']","iframes":true,"iframesTimeout":4999,"ignoreGroups":1,"debug":true,"queryRegExp":"/()\\\\b(query|test)\\\\b/gi","selectors":"p,li","selectorAll":true,"testString":{"mode":"html","content":"<p>Test string.</p>"}}}`,
			`{"section":{"type":"regexp","acrossElements":true,"separateGroups":true,"wrapAllRanges":true,"queryRegExp":"/\\\\b(query|test)\\\\b/gi","testString":{"mode":"html","content":"<p>Test string.</p>"}}}`
		],
		ranges : [
			`{"section":{"type":"ranges","element":"span","className":"test","exclude":"['h1','.ignore']","iframes":true,"iframesTimeout":4999,"wrapAllRanges":true,"shadowDOM":"{style:'mark[data-markjs]{color:red;}'}","debug":true,"queryRanges":"[{'start':0,'length':5},{'start':6,'length':6}]","selectors":"p,li","selectorAll":true,"testString":{"mode":"html","content":"<p>Test string.</p>"}}}`,
			`{"section":{"type":"ranges","iframes":true,"wrapAllRanges":true,"shadowDOM":true,"queryRanges":"[{'start':0,'length':5},{'start':6,'length':6}]","testString":{"mode":"html","content":"<p>Test string.</p>"}}}`,
			
			`{"section":{"type":"ranges","queryRanges":"[{'start':0,'length':5},{'start':6,'length':6}]","selectors":"p","selectorAll":false,"testString":{"mode":"html","content":"<p>Test string.</p>"}}}`,
		]
	},
		...
		for(const type in types) {
			...
			this.jsons[type].forEach(str => {
				...
				if(currentLibrary.old) {
					// removes mark.js new options from the object to test the old library
		...
		for(const type in types) {
			...
			this.jsons[type].every(json => {
				if(obj = Json.parseJson(json)) {
					...
					let code = codeBuilder.build(currentLibrary.jquery ? 'jq' : 'js');
	...
	checkDefaultValues : function(internal) {
		...
		if(internal) {
			...
			for(const name in defaultOptions) {
				if(currentLibrary.old && newOptions.includes(name)) continue;
				
				let value = options[name];
				
				// the internal code shouldn't have any option except callbacks
				if( !isNullOrUndefined(value)) {
					console.log(`The option "${name} : ${value}" is not reset to default value ${defaultOptions[name].value}. ${currentType}`);
					success = false;
				}
			}
			...
			for(const name in defaultOptions) {
				...
				if( !isNullOrUndefined(value) && value !== defaultValue) {
					...
					console.log(`The option "${name} : ${value}" is not reset to default value ${defaultValue}. ${currentType}, internal test=${internal}`);
					success = false;
				}
			}
		}
	...
	checkSetOptions : function(options, obj, internal) {
		...
		for(const name in obj.section) {
			...
			if(isNullOrUndefined(setValue)) {
				console.log(`The option ${name} value is ${value}. ${currentType}`);
				success = false;
				continue;
			}
			...
			if(typeof setValue === 'object') {
				...
				if(value) {
					...
					console.log(`The option '${name}' values are not equals ${value} != ${setValue}. ${currentType}`);
					success = false;
